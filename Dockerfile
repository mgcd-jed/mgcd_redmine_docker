...
# if we use wget here, we get certificate issues (https://github.com/docker-library/redmine/pull/249#issuecomment-984176479)
	...
	# Download our patches git
	# If this breaks, start git cloning a specific commit of redmine:master :shrug:
	mkdir mgcd_redmine_docker; \
	git clone https://github.com/mgcd-jed/mgcd_redmine_docker.git ./mgcd_redmine_docker; \

	# Use Fred's workspaces patch (issue #20384)
	git apply ./mgcd_redmine_docker/workspaces_redmine_5.0_all.patch; \

	# 4939's patch to have more filtering options
	git apply ./mgcd_redmine_docker/redmine5.0.1-or-query-patch.patch; \
